...
fn main() {
    ...
    // -------------------------------------------------------------------------
    let fixed_n=1000;
    let mut list_edges = file_reader::read_file("t_twitter.txt");
    list_edges.sort();

    // Calculate the Page Rank of our list_edges
    let mut page_rank = page_rank(&mut list_edges, fixed_n);

    // Filtering list_edges to only contain the connections between the top 50 edges
    ...
    let adj_list = grouped_vertex_tuples(&list_edges);
    println!("Length of Graph: {:?}\n",adj_list.len());
    ...
    for i in 0..top50.len() {
        let distance_nodes= bfs_distance_nodes(&graph_hashmap, top50[0], top50[i]);
...
mod tests {
    ...
    fn test_read_file_success() {
        let edges = read_file("test_t_twitter.txt");

        assert_eq!(!edges.is_empty(), true);
    }

    #[test]
    fn test_bfs_distance_nodes_success() {
        let mut graph_hashmap = HashMap::new();

        let edges_1 = vec![2, 3];
        graph_hashmap.insert(1, &edges_1);

        let edges_2 = vec![1, 4];
        graph_hashmap.insert(2, &edges_2);

        let edges_3 = vec![1, 5];
        graph_hashmap.insert(3, &edges_3);

        let edges_4 = vec![2];
        graph_hashmap.insert(4, &edges_4);

        let edges_5 = vec![3, 6];
        graph_hashmap.insert(5, &edges_5);

        let edges_6 = vec![5];
        graph_hashmap.insert(6, &edges_6);
    }
